...
def sidebar():
    with st.sidebar:
        ...
        if role == "admin":
            menu_items["⚙️ User Management"] = "Users"
...
def page_dashboard():
    ...
    with col3:
        st.markdown("### 🧑‍⚕️ Recently Registered Patients")
        if patients:
            df = pd.DataFrame([{
                "ID": p["patient_id"],
                "Name": p["full_name"],
...
def page_patients():
    ...
    tabs = st.tabs(["📋 All Patients", "➕ Add Patient", "✏️ Edit Patient", "🗑️ Delete Patient", "📊 Patient History"])
    ...
    with tabs[3]:
        ...
        if patients_all:
            ...
            with col_a:
                if st.button("⚠️ Deactivate (Soft Delete)", use_container_width=True):
...
def page_doctors():
    st.markdown("<div class='section-header'>👨‍⚕️ Doctor Management</div>", unsafe_allow_html=True)

    tabs = st.tabs(["📋 All Doctors", "➕ Add Doctor", "✏️ Edit Doctor", "🗑️ Delete Doctor"])
...
def page_appointments():
    ...
    tabs = st.tabs(["📋 All Appointments", "➕ Book Appointment", "✏️ Edit Appointment", "❌ Cancel / Delete"])
    ...
    with tabs[3]:
        ...
        if appts_all:
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("⚠️ Mark as Cancelled", use_container_width=True):
...
def page_prescriptions():
    ...
    tabs = st.tabs(["📋 All Prescriptions", "➕ Issue Prescription", "🗑️ Delete Prescription"])
    ...
    with tabs[2]:
        ...
        if rxs:
            ...
            if st.button("🗑️ Delete Prescription", type="primary"):
...
def page_billing():
    ...
    tabs = st.tabs(["📋 All Bills", "➕ Generate Bill", "💰 Update Payment", "🗑️ Delete Bill", "📊 Revenue Report"])
    ...
    with tabs[3]:
        ...
        if bills:
            ...
            bid = options[selected]
            if st.button("🗑️ Delete Bill", type="primary"):
                Bill.delete(bid)
                log("DELETE_BILL", bid)
...
def page_users():
    ...
    st.markdown("<div class='section-header'>⚙️ User Management</div>", unsafe_allow_html=True)

    tabs = st.tabs(["👥 All Users", "➕ Add User", "🗑️ Delete User", "🔑 Change Password"])
